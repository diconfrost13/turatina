...
ACMD_FUNC(bodystyle)
{
	...
	// @bodystyle Mod [Gidz]
	if (sd->class_ == MAPID_NOVICE ||				sd->class_ == MAPID_SWORDMAN ||			sd->class_ == MAPID_MAGE ||					sd->class_ == MAPID_ARCHER ||
		sd->class_ == MAPID_ACOLYTE ||				sd->class_ == MAPID_MERCHANT ||			sd->class_ == MAPID_THIEF ||				sd->class_ == MAPID_TAEKWON ||
		sd->class_ == MAPID_WEDDING ||				sd->class_ == MAPID_XMAS ||				sd->class_ == MAPID_SUMMER ||				sd->class_ == MAPID_HANBOK ||
		sd->class_ == MAPID_GANGSI ||				sd->class_ == MAPID_OKTOBERFEST ||		sd->class_ == MAPID_SUMMONER ||				sd->class_ == MAPID_SUMMER2 ||
		sd->class_ == MAPID_KNIGHT ||				sd->class_ == MAPID_WIZARD ||			sd->class_ == MAPID_HUNTER ||				sd->class_ == MAPID_PRIEST ||
		sd->class_ == MAPID_BLACKSMITH ||			sd->class_ == MAPID_ASSASSIN ||			sd->class_ == MAPID_REBELLION ||			sd->class_ == MAPID_KAGEROUOBORO ||
		sd->class_ == MAPID_DEATH_KNIGHT ||			sd->class_ == MAPID_DARK_COLLECTOR ||	sd->class_ == MAPID_NOVICE_HIGH ||			sd->class_ == MAPID_SWORDMAN_HIGH ||
		sd->class_ == MAPID_MAGE_HIGH ||			sd->class_ == MAPID_ARCHER_HIGH ||		sd->class_ == MAPID_ACOLYTE_HIGH ||			sd->class_ == MAPID_MERCHANT_HIGH ||
		sd->class_ == MAPID_THIEF_HIGH ||			sd->class_ == MAPID_SUPER_NOVICE_E ||	sd->class_ == MAPID_STAR_EMPEROR ||			sd->class_ == MAPID_SOUL_REAPER ||
		sd->class_ == MAPID_BABY_SWORDMAN ||		sd->class_ == MAPID_BABY_MAGE ||		sd->class_ == MAPID_BABY_ARCHER ||			sd->class_ == MAPID_BABY_ACOLYTE ||
		sd->class_ == MAPID_BABY_MERCHANT ||		sd->class_ == MAPID_BABY_THIEF ||		sd->class_ == MAPID_BABY_TAEKWON ||			sd->class_ == MAPID_BABY_SUMMONER ||
		sd->class_ == MAPID_BABY_KNIGHT ||			sd->class_ == MAPID_BABY_WIZARD ||		sd->class_ == MAPID_BABY_HUNTER ||			sd->class_ == MAPID_BABY_PRIEST ||
		sd->class_ == MAPID_BABY_BLACKSMITH ||		sd->class_ == MAPID_BABY_ASSASSIN ||	sd->class_ == MAPID_BABY_STAR_GLADIATOR ||	sd->class_ == MAPID_BABY_REBELLION ||
		sd->class_ == MAPID_BABY_KAGEROUOBORO ||	sd->class_ == MAPID_SUPER_BABY_E ||		sd->class_ == MAPID_BABY_STAR_EMPEROR ||	sd->class_ == MAPID_BABY_SOUL_REAPER) {
