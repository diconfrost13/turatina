...
PlayerBonusDatabase player_bonus_db;
...
void pc_player_bonus(struct map_session_data *sd) {
	int idx = 0, i = 0;
	std::shared_ptr<s_player_bonus_db> js;

	nullpo_retv(sd);
	while ( (js = player_bonus_db.find(i)) != nullptr ) {
		if(	js->type == JST_ALL
...
const std::string PlayerBonusDatabase::getDefaultLocation() {
	return std::string(db_path) + "/player_bonus.yml";
}

uint64 PlayerBonusDatabase::parseBodyNode( const YAML::Node& node ){
	uint32 id;

	if (!this->asUInt32(node, "Id", id))
		return 0;

	std::shared_ptr<s_player_bonus_db> js = this->find(id);
	...
	if (!exists) {
		...
		js = std::make_shared<s_player_bonus_db>();
		js->id = id;
	}
